...
async function requireProAccess() {
  ...
  if (!(await isProUser(user.id))) throw new Error("SmithAI is a Pro feature.");
  return user;
}
...
export async function sendAiMessageAction(input: { content: string }) {
  ...
  if (!aiIsConfigured()) {
    throw new Error(
      "SmithAI isn't configured yet — ask your NumberSmith admin to set ANTHROPIC_API_KEY."
    );
  }
  ...
  try {
    ...
    throw new Error("SmithAI couldn't respond just now — try again in a moment.");
  }
