...
export default async function CompetitionDetailPage({
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      <Link href="/competitions" className="text-sm font-medium text-slate-400 hover:text-slate-600">
      ...
      <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold text-slate-900 sm:text-3xl">{competition.shortName}</h1>
            {userCompetition && (
          ...
          <p className="mt-1 text-sm text-slate-500">{competition.name}</p>
        </div>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-3">
        <div className="space-y-5 lg:col-span-2">
          <Card>
            <CardBody>
              <h2 className="text-sm font-bold uppercase tracking-wide text-slate-400">Overview</h2>
              <p className="mt-2 text-sm leading-relaxed text-slate-600">{competition.description}</p>
              <p className="mt-3 text-xs text-slate-400">{competition.organization}</p>
            </CardBody>
          </Card>

          <Card>
            <CardBody>
              <h2 className="text-sm font-bold uppercase tracking-wide text-slate-400">Format</h2>
                    ...
                    <div key={ct.id}>
                      <div className="mb-1 flex items-center justify-between text-sm">
                        <span className="flex items-center gap-2 font-medium text-slate-700">
                          {ct.topic.name}
                          <span className="text-[11px] text-slate-400">weight {ct.weight}/5</span>
                        </span>
          ...
          <Card>
            <CardBody>
              ...
              <p className="mt-2 text-sm leading-relaxed text-slate-600">{competition.roadmap}</p>
            </CardBody>
          </Card>
              ...
              <CardBody>
                ...
                <div className="mt-3 divide-y divide-slate-100">
                  {problems.map((p) => (
                    ...
                    >
                      <span className="line-clamp-2 text-sm text-slate-600">{p.question}</span>
                ...
                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                  {lessons.map((l) => (
                    <Link
                      key={l.id}
                      href={`/lessons/${l.slug}`}
                      className="rounded-xl border border-slate-200 px-3 py-2.5 hover:border-brand-300"
                    >
                      <p className="text-sm font-semibold text-slate-800">{l.title}</p>
                      <p className="mt-0.5 text-xs text-slate-400">{l.topic.name}</p>
                    </Link>
                  ))}
                </div>
              </CardBody>
        ...
        <div className="space-y-5">
          <Card>
            <CardBody className="text-center">
              ...
              <p className="mt-2 text-4xl font-extrabold text-brand-700">{overallMastery}%</p>
              <ProgressBar value={overallMastery} tone="success" className="mt-3" />
              <p className="mt-2 text-xs text-slate-400">Weighted across this competition&apos;s topics</p>
            </CardBody>
          </Card>
