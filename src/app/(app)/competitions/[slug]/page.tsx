...
export default async function CompetitionDetailPage({
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      <Link href="/competitions" className="text-sm font-medium text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300">
                    ...
                    <div key={ct.id}>
                      <div className="mb-1 flex items-center justify-between text-sm">
                        <span className="flex items-center gap-2 font-medium text-slate-700 dark:text-slate-200">
                          {ct.topic.name}
                          <span className="text-[11px] text-slate-400 dark:text-slate-500">weight {ct.weight}/5</span>
                        </span>
                ...
                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                  {lessons.map((l) => (
                    ...
                    >
                      ...
                      <p className="mt-0.5 text-xs text-slate-400 dark:text-slate-500">{l.topic.name}</p>
                    </Link>
                  ))}
                </div>
              </CardBody>
        ...
        <div className="space-y-5">
          <Card>
            <CardBody className="text-center">
              ...
              <p className="mt-2 text-xs text-slate-400 dark:text-slate-500">Weighted across this competition&apos;s topics</p>
            </CardBody>
          </Card>
...
function Fact({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <dt className="text-[11px] font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500">{label}</dt>
