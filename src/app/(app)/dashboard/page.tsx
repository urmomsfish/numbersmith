        ...
        <div className="space-y-5 lg:col-span-2">
          <Card>
            <CardBody>
              ...
              <div className="mt-3 grid grid-cols-3 gap-3 text-center">
                <div>
                  <p className="text-2xl font-extrabold text-slate-900">{problemCount}</p>
                ...
                <div>
                  <p className="text-2xl font-extrabold text-slate-900">80%+</p>
                  <p className="text-xs text-slate-400">accuracy</p>
                </div>
              </div>
              {today && (
                <p className="mt-4 rounded-lg bg-slate-50 px-3 py-2 text-center text-xs text-slate-500">
                  Today&apos;s plan: <span className="font-semibold text-slate-700">{today.label}</span>
                </p>
              )}
            </CardBody>
          </Card>
          ...
          <Card>
            <CardBody>
              ...
              <p className="mt-2 text-xl font-bold text-slate-900">{priorityTopic.name}</p>
              <div className="mt-2 flex items-center gap-2">
                <ProgressBar value={priorityMastery} tone="brand" className="max-w-[200px]" />
                <span className="text-xs font-medium text-slate-400">{priorityMastery}% mastered</span>
              </div>
              ...
              <div className="mt-3 space-y-3">
                {competitionProgress.length === 0 && (
                  <p className="text-sm text-slate-400">
                    ...
                    <Link href="/settings/competitions" className="text-brand-600 underline">
                ...
                {competitionProgress.map((cp) => (
                  <div key={cp.competition.id}>
                    <div className="mb-1 flex items-center justify-between text-sm">
                      ...
                      <span className="text-slate-500">{cp.pct}% mastered</span>
                    </div>
        ...
        <div className="space-y-5">
          <Card>
            <CardBody className="text-center">
              ...
              <p className="mt-2 text-4xl font-extrabold text-brand-700">{ratingValue}</p>
          ...
          <Card>
            <CardBody className="text-center">
              ...
              <p className="mt-2 flex items-center justify-center gap-2 text-3xl font-extrabold text-ember-600">
                <IconFlame className="h-7 w-7" />
                {stats?.currentStreak ?? 0} days
              </p>
              <p className="mt-1 text-xs text-slate-400">Longest: {stats?.longestStreak ?? 0} days</p>
            </CardBody>
          </Card>
